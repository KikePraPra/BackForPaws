...
<body class="bg-white">
    <main class="bg-white min-h-screen flex items-center justify-center">
        <article class="min-h-screen flex items-center justify-center flex-col rounded-3xl bg-white px-4 sm:px-7 pt-3 z-10 w-full">
            <h1 class="text-3xl font-bold text-green-700 mb-8 text-center w-full">Adopciones</h1>
            ...
            <div class="w-full max-w-md">
                <ul class="flex flex-col gap-4 w-full">
                    @forelse ($adoptions as $adoption)
                        <li class="text-green-900 rounded-lg px-4 py-3 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                            <div class="flex-1 min-w-0">
                                ...
                                <a href="{{ route('adoption.show', $adoption->id) }}"
                            ...
                            <div class="flex gap-2 mt-2 sm:mt-0 flex-shrink-0">
                                ...
                                <form action="{{ route('adoption.destroy', $adoption->id) }}" method="POST" onsubmit="return confirm('Está seguro que quieres eliminar esta adopción?');">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
                    ...
                    @empty
                        <li class="text-center text-gray-500">There are no registered adoptions.</li>
                    @endforelse
                </ul>
            </div>
        </article>
