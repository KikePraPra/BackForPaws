...
<body>
    <main class="bg-white">
        <article class="min-h-screen flex items-center justify-center flex-col rounded-3xl bg-white px-7 pt-10 z-10">
            <h1 class="text-3xl font-bold text-green-700">Editar adopción</h1>
            <div class="mt-11 flex flex-col gap-7">
                ...
                <form method="POST" action="{{ route('adoption.update', $adoption->id) }}" enctype="multipart/form-data" class="flex flex-col w-max mx-auto font-sans">
                    @csrf
                    @method('PUT')
                    ...
                    <select name="account_id" required class="rounded-lg border-2 border-green-600 h-12 w-80 mt-2 pl-2 text-lg font-semibold">
                        ...
                        @foreach($accounts as $account)
                            <option value="{{ $account->id }}" {{ old('account_id', $adoption->account_id) == $account->id ? 'selected' : '' }}>
                                {{ $account->username }}
                            </option>
                        @endforeach
                    </select>
                    ...
                    <select name="animal_id" required class="rounded-lg border-2 border-green-600 h-12 w-80 mt-2 pl-2 text-lg font-semibold">
                        ...
                        @foreach($animals as $animal)
                            <option value="{{ $animal->id }}" {{ old('animal_id', $adoption->animal_id) == $animal->id ? 'selected' : '' }}>
                    ...
                    <input type="text" name="pet_name" value="{{ old('pet_name', $adoption->pet_name) }}" required
                    ...
                    <input type="number" name="pet_age" value="{{ old('pet_age', $adoption->pet_age) }}" min="0"
                        class="rounded-lg border-2 border-green-600 h-12 w-80 mt-2 pl-2 text-lg font-semibold"/>
                    ...
                    <select name="pet_state" required class="rounded-lg border-2 border-green-600 h-12 w-80 mt-2 pl-2 text-lg font-semibold">
                        ...
                        <option value="en_adopción" {{ old('pet_state', $adoption->pet_state) == 'en_adopción' ? 'selected' : '' }}>En adopción</option>
                        <option value="adoptado" {{ old('pet_state', $adoption->pet_state) == 'adoptado' ? 'selected' : '' }}>Adoptado</option>
                    </select>

                    <label class="mt-6">Descripción:</label>
                    <textarea name="description" required rows="3"
                        class="rounded-lg border-2 border-green-600 w-80 mt-2 pl-2 text-lg font-semibold resize-none">{{ old('description', $adoption->description) }}</textarea>
                    ...
                    <div class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-md hover:border-green-500 transition duration-150">
                        <div class="space-y-1 text-center">
                            <div id="imagePreviewContainer" class="{{ $adoption->pet_image ? '' : 'hidden' }} mb-4">
                                <img id="imagePreview"
                                     src="{{ $adoption->pet_image ? asset('storage/' . $adoption->pet_image) : '' }}"
                                     class="mx-auto max-h-48 rounded-lg shadow-lg"
                                     alt="Image preview">
                            </div>
                            <svg id="uploadIcon" class="mx-auto h-12 w-12 text-gray-400 {{ $adoption->pet_image ? 'hidden' : '' }}" stroke="currentColor" fill="none" viewBox="0 0 48 48" aria-hidden="true">
